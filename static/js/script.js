...
function showRecommendationModal(recommendationsUrl) {
    ...
    document.getElementById("stay-btn").addEventListener("click", () => {
        ...
        // Fetch estimated recommendation generation time
        const estimatedGenerationTime = 15000; // Replace with your dynamic calculation or a fetch value in milliseconds

        // Schedule the modal to reappear after the estimated generation time
        setTimeout(() => {
            if (document.getElementById("game-overlay").style.display === "flex") {
                // Ensure the game overlay is still active before showing the modal again
